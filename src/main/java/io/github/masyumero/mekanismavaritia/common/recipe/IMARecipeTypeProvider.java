package io.github.masyumero.mekanismavaritia.common.recipe;
...
public interface IMARecipeTypeProvider<RECIPE extends MekanismRecipe, INPUT_CACHE extends IInputRecipeCache>{
    default ResourceLocation getRegistryName() {
        return getMARecipeType().getRegistryName();
    }

    MARecipeType<RECIPE, INPUT_CACHE> getMARecipeType();

    default INPUT_CACHE getInputCache() {
        return getMARecipeType().getInputCache();
    }

    @NotNull
    default List<RECIPE> getRecipes(@Nullable Level world) {
        return getMARecipeType().getRecipes(world);
    }
