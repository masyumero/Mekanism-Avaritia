...
import io.github.masyumero.mekanismavaritia.common.inventory.slot.MAFactoryOutputInventorySlot;
...
public class TileEntitySawingMAFactory extends TileEntityMAFactory<SawmillRecipe> implements ISingleRecipeLookupHandler.ItemRecipeLookupHandler<SawmillRecipe> {
    ...
    public TileEntitySawingMAFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            MAFactoryOutputInventorySlot outputSlot = MAFactoryOutputInventorySlot.at(this, updateSortingListener, xPos, 57);
            MAFactoryOutputInventorySlot secondaryOutputSlot = MAFactoryOutputInventorySlot.at(this, updateSortingListener, xPos, 77);
            //Note: As we are an item factory that has comparator's based on items we can just use the monitor as a listener directly
            MAFactoryInputInventorySlot inputSlot = MAFactoryInputInventorySlot.create(this, i, outputSlot, secondaryOutputSlot, recipeCacheLookupMonitors[i], xPos, 13);
            int index = i;
