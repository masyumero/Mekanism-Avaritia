...
public class RenderMALogisticalTransporter extends RenderTransmitterBase<TileEntityMALogisticalTransporterBase> {
    ...
    public RenderMALogisticalTransporter(BlockEntityRendererProvider.Context context) {
        super(context);
    ...
    @Override
    protected void render(TileEntityMALogisticalTransporterBase tile, float partialTick, PoseStack matrix, MultiBufferSource renderer, int light, int overlayLight,
                          ProfilerFiller profiler) {
        MALogisticalTransporter transporter = (MALogisticalTransporter) tile.getTransmitter();
        ...
        if (!inTransit.isEmpty()) {
            ...
            float partial = partialTick * MATPTier.getSpeed(transporter.tier);

            Collection<TransporterStack> reducedTransit = getReducedTransit(inTransit);
            for (TransporterStack stack : reducedTransit) {
                float[] stackPos = MATransporterUtils.getStackPosition(transporter, stack, partial);
                matrix.pushPose();
