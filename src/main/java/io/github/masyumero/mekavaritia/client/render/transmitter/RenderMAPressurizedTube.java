...
public class RenderMAPressurizedTube extends RenderTransmitterBase<TileEntityMAPressurizedTube> {

    public RenderMAPressurizedTube(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void render(TileEntityMAPressurizedTube tile, float partialTick, PoseStack matrix, MultiBufferSource renderer, int light, int overlayLight,
    ...
    @Override
    protected boolean shouldRenderTransmitter(TileEntityMAPressurizedTube tile, Vec3 camera) {
        if (super.shouldRenderTransmitter(tile, camera)) {
            MABoxedPressurizedTube tube = tile.getTransmitter();
