...
public class MABoxedPressurizedTube extends BoxedPressurizedTube implements IMAUpgradeableTransmitter<PressurizedTubeUpgradeData> {

    public MABoxedPressurizedTube(IBlockProvider blockProvider, TileEntityTransmitter tile) {
    ...
    private long getAvailablePull(ChemicalType chemicalType) {
        if (hasTransmitterNetwork()) {
            return Math.min(MATTier.getTubePullAmount(this.tier), getTransmitterNetwork().chemicalTank.getTankForType(chemicalType).getNeeded());
        }
        return Math.min(MATTier.getTubePullAmount(this.tier), chemicalTank.getTankForType(chemicalType).getNeeded());
    }

    @Override
    public long getCapacity() {
        return MATTier.getTubeCapacity(this.tier);
    }
