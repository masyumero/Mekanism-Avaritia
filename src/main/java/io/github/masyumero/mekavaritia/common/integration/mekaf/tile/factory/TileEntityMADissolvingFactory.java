...
import io.github.masyumero.mekavaritia.common.integration.mekaf.tile.factory.base.TileEntityMAItemToMergedFactory;
...
public class TileEntityMADissolvingFactory extends TileEntityMAItemToMergedFactory<ChemicalDissolutionRecipe> implements IHasDumpButton, ItemChemicalRecipeLookupHandler<Gas, GasStack, ChemicalDissolutionRecipe> {
    ...
    public TileEntityMADissolvingFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalDissolvingInput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralDissolvingInput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareDissolvingInput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticDissolvingInput.get();
            };
        } else {
