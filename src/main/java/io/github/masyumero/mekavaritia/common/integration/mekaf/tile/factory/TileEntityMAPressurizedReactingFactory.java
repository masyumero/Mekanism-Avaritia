package io.github.masyumero.mekavaritia.common.integration.mekaf.tile.factory;

import com.jerry.mekaf.common.upgrade.PRCUpgradeData;
import io.github.masyumero.mekavaritia.common.config.LoadConfig;
import io.github.masyumero.mekavaritia.common.integration.mekaf.inventory.slot.MAAdvancedFactoryInputInventorySlot;
...
public class TileEntityMAPressurizedReactingFactory extends TileEntityMAAdvancedFactoryBase<PressurizedReactionRecipe> implements IHasDumpButton, ItemFluidChemicalRecipeLookupHandler<Gas, GasStack, PressurizedReactionRecipe> {
    ...
    public TileEntityMAPressurizedReactingFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    private long getInputTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalPRCInput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralPRCInput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flarePRCInput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticPRCInput.get();
            };
        } else {
    ...
    private long getOutputTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalPRCOutput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralPRCOutput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flarePRCOutput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticPRCOutput.get();
            };
        } else {
            return MAX_GAS * tier.processes;
        }
    ...
    private int getInputFluidTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalPRCFluidInput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralPRCFluidInput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flarePRCFluidInput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticPRCFluidInput.get();
            };
        } else {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            MAAdvancedFactoryInputInventorySlot inputSlot = MAAdvancedFactoryInputInventorySlot.create(this, i, outputSlot, outputGasTank, recipeCacheLookupMonitors[i], getXPos(i), 13);
            int index = i;
    ...
    private void distributeItems(Map<HashedItem, PRCRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, PRCRecipeProcessInfo> entry : processes.entrySet()) {
            ...
            for (int i = 0; i < processCount; i++) {
                ...
                MAAdvancedFactoryInputInventorySlot inputSlot = processInfo.inputSlot();
    ...
    public record PRCProcessInfo(int process, @NotNull MAAdvancedFactoryInputInventorySlot inputSlot,
