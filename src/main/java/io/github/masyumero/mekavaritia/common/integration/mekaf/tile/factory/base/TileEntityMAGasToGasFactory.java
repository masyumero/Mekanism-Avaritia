...
public abstract class TileEntityMAGasToGasFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {
    ...
    protected TileEntityMAGasToGasFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    private long getInputTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalCentrifugingInput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralCentrifugingInput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareCentrifugingInput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticCentrifugingInput.get();
            };
        } else {
    ...
    private long getOutputTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalCentrifugingOutput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralCentrifugingOutput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareCentrifugingOutput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticCentrifugingOutput.get();
            };
        } else {
            return MAX_CHEMICAL * tier.processes;
        }
