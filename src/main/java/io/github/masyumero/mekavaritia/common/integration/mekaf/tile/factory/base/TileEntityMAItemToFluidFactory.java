...
public abstract class TileEntityMAItemToFluidFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {
    ...
    protected TileEntityMAItemToFluidFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    private int getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalLiquifying.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralLiquifying.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareLiquifying.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticLiquifying.get();
            };
        } else {
    ...
    public record ItemToFluidProcessInfo(int process, @NotNull MAAdvancedFactoryInputInventorySlot inputSlot,
