...
public abstract class TileEntityMAItemToGasFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {
    ...
    protected TileEntityMAItemToGasFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalOxidizing.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralOxidizing.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareOxidizing.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticOxidizing.get();
            };
        } else {
    ...
    public record ItemToGasProcessInfo(int process, @NotNull MAAdvancedFactoryInputInventorySlot inputSlot,
