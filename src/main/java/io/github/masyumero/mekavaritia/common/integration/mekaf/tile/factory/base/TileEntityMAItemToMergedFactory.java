...
public abstract class TileEntityMAItemToMergedFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {

    private static final long MAX_CHEMICAL = 10_000;

    protected ItemToMergedProcessInfo[] processInfoSlots;
    MAAdvancedFactoryInputInventorySlot[] inputSlot;
    ...
    protected TileEntityMAItemToMergedFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    protected void presetVariables() {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            mergedOutputHandlers[i] = new BoxedChemicalOutputHandler(outputTank[i], RecipeError.NOT_ENOUGH_OUTPUT_SPACE);
        }
    }
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalDissolvingOutput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralDissolvingOutput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareDissolvingOutput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticDissolvingOutput.get();
            };
        } else {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        inputSlot = new MAAdvancedFactoryInputInventorySlot[tier.processes];
        itemInputHandlers = new IInputHandler[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            inputSlot[i] = MAAdvancedFactoryInputInventorySlot.create(this, i, outputTank[i], recipeCacheLookupMonitors[i], getXPos(i), 13);
            int index = i;
    ...
    public record ItemToMergedProcessInfo(int process, @NotNull MAAdvancedFactoryInputInventorySlot inputSlot,
