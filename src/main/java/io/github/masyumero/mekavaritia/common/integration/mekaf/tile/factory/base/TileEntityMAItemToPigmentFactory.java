...
public abstract class TileEntityMAItemToPigmentFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {
    ...
    protected TileEntityMAItemToPigmentFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalPigmentExtracting.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralPigmentExtracting.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flarePigmentExtracting.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticPigmentExtracting.get();
            };
        } else {
    ...
    public record ItemToPigmentProcessInfo(int process, @NotNull MAAdvancedFactoryInputInventorySlot inputSlot,
