...
public abstract class TileEntityMAMergedToItemFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {

    protected static final long MAX_CHEMICAL = 10_000;

    protected MergedToItemProcessInfo[] processInfoSlots;
    protected MAAdvancedFactoryOutputInventorySlot[] outputSlot;
    ...
    protected TileEntityMAMergedToItemFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        outputSlot = new MAAdvancedFactoryOutputInventorySlot[tier.processes];
        itemOutputHandlers = new IOutputHandler[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            outputSlot[i] = MAAdvancedFactoryOutputInventorySlot.at(this, recipeCacheLookupMonitors[i], getXPos(i), 70);
            int index = i;
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalCrystallizing.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralCrystallizing.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareCrystallizing.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticCrystallizing.get();
            };
        } else {
