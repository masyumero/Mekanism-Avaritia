...
public abstract class TileEntityMASlurryToSlurryFactory<RECIPE extends MekanismRecipe> extends TileEntityMAAdvancedFactoryBase<RECIPE> {
    ...
    protected TileEntityMASlurryToSlurryFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
    ...
    protected void addSlurryTanks(ChemicalTankHelper<Slurry, SlurryStack, ISlurryTank> builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            slurryInputHandlers[i] = InputHelper.getInputHandler(inputTank[i], RecipeError.NOT_ENOUGH_INPUT);
            slurryOutputHandlers[i] = OutputHelper.getOutputHandler(outputTank[i], RecipeError.NOT_ENOUGH_OUTPUT_SPACE);
    ...
    private long getInputTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalWashingInput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralWashingInput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareWashingInput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticWashingInput.get();
            };
        } else {
    ...
    private long getOutputTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalWashingOutput.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralWashingOutput.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareWashingOutput.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticWashingOutput.get();
            };
        } else {
            return MAX_CHEMICAL * tier.processes;
        }
