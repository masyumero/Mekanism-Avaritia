...
import io.github.masyumero.mekavaritia.common.integration.mekmm.inventory.slot.MAMoreMachineFactoryOutputInventorySlot;
...
public class TileEntityMAPlantingFactory extends TileEntityMAMoreMachineFactory<PlantingRecipe> implements IBoundingBlock, IHasDumpButton, ConstantUsageRecipeLookupHandler,
    ...
    public TileEntityMAPlantingFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalPlanting.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralPlanting.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flarePlanting.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticPlanting.get();
            };
        } else {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            MAMoreMachineFactoryOutputInventorySlot outputSlot = MAMoreMachineFactoryOutputInventorySlot.at(this, updateSortingListener, getXPos(i), 57);
            MAMoreMachineFactoryOutputInventorySlot secondaryOutputSlot = MAMoreMachineFactoryOutputInventorySlot.at(this, updateSortingListener, getXPos(i), 77);
            // Note: As we are an item factory that has comparator's based on items we can just use the monitor as a
            // listener directly
            MAMoreMachineFactoryInputInventorySlot inputSlot = MAMoreMachineFactoryInputInventorySlot.create(this, i, outputSlot, secondaryOutputSlot, recipeCacheLookupMonitors[i], getXPos(i), 13);
            int index = i;
    ...
    @Nullable
    @Override
    public GasInventorySlot getMASlot() {
        return gasSlot;
    }
