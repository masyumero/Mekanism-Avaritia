...
public class TileEntityMAReplicatingFactory extends TileEntityMAItemToItemFactory<ItemStackGasToItemStackRecipe> implements IHasDumpButton,
    ...
    public TileEntityMAReplicatingFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    public @Nullable IChemicalTankHolder<Gas, GasStack, IGasTank> getInitialGasTanks(IContentsListener listener) {
        ...
        gasTank = ChemicalTankBuilder.GAS.create(getTankCapacity(), TileEntityMAReplicatingFactory::isValidChemicalInput, markAllMonitorsChanged(listener));
    ...
    private long getTankCapacity() {
        if (LoadConfig.MA_MORE_CAPACITY_CONFIG.moreCapacityMode.get()) {
            return switch (tier) {
                case ETERNAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.eternalReplicating.get();
                case NEURAL -> LoadConfig.MA_MORE_CAPACITY_CONFIG.neuralReplicating.get();
                case FLARE -> LoadConfig.MA_MORE_CAPACITY_CONFIG.flareReplicating.get();
                case PRISMATIC -> LoadConfig.MA_MORE_CAPACITY_CONFIG.prismaticReplicating.get();
            };
        } else {
