...
import io.github.masyumero.mekavaritia.common.content.network.transmitter.MAMechanicalPipe;
...
public class TileEntityMAMechanicalPipe extends TileEntityMATransmitter implements IComputerTile {

    private final FluidHandlerManager fluidHandlerManager;

    public TileEntityMAMechanicalPipe(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
        super(blockProvider, pos, state);
        addCapabilityResolver(fluidHandlerManager = new FluidHandlerManager(direction -> {
            MAMechanicalPipe pipe = getTransmitter();
    ...
    @Override
    protected MAMechanicalPipe createTransmitter(IBlockProvider blockProvider) {
        return new MAMechanicalPipe(blockProvider, this);
    }

    @Override
    public MAMechanicalPipe getTransmitter() {
        return (MAMechanicalPipe) super.getTransmitter();
    }
    ...
    protected BlockState upgradeResult(@NotNull BlockState current, @NotNull MATier tier) {
        return BlockStateHelper.copyStateData(current, switch (tier) {
            case PRISMATIC -> MABlocks.PRISMATIC_MECHANICAL_PIPE;
            case FLARE -> MABlocks.FLARE_MECHANICAL_PIPE;
            case NEURAL -> MABlocks.NEURAL_MECHANICAL_PIPE;
            case ETERNAL -> MABlocks.ETERNAL_MECHANICAL_PIPE;
        });
    }
