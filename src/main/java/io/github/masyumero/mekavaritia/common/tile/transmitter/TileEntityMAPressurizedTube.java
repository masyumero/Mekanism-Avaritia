...
import io.github.masyumero.mekavaritia.common.content.network.transmitter.MABoxedPressurizedTube;
...
public class TileEntityMAPressurizedTube extends TileEntityMATransmitter implements IComputerTile, ITileRadioactive {
    ...
    public TileEntityMAPressurizedTube(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
        ...
        addCapabilityResolver(gasHandlerManager = new ChemicalHandlerManager.GasHandlerManager(getHolder(MABoxedPressurizedTube::getGasTanks),
                new DynamicChemicalHandler.DynamicGasHandler(this::getGasTanks, canExtract, canInsert, null)));
        addCapabilityResolver(infusionHandlerManager = new ChemicalHandlerManager.InfusionHandlerManager(getHolder(MABoxedPressurizedTube::getInfusionTanks),
                new DynamicChemicalHandler.DynamicInfusionHandler(this::getInfusionTanks, canExtract, canInsert, null)));
        addCapabilityResolver(pigmentHandlerManager = new ChemicalHandlerManager.PigmentHandlerManager(getHolder(MABoxedPressurizedTube::getPigmentTanks),
                new DynamicChemicalHandler.DynamicPigmentHandler(this::getPigmentTanks, canExtract, canInsert, null)));
        addCapabilityResolver(slurryHandlerManager = new ChemicalHandlerManager.SlurryHandlerManager(getHolder(MABoxedPressurizedTube::getSlurryTanks),
    ...
    @Override
    protected MABoxedPressurizedTube createTransmitter(IBlockProvider blockProvider) {
        return new MABoxedPressurizedTube(blockProvider, this);
    }

    @Override
    public MABoxedPressurizedTube getTransmitter() {
        return (MABoxedPressurizedTube) super.getTransmitter();
    }
    ...
    protected BlockState upgradeResult(@NotNull BlockState current, @NotNull MATier tier) {
        return BlockStateHelper.copyStateData(current, switch (tier) {
            case PRISMATIC -> MABlocks.PRISMATIC_PRESSURIZED_TUBE;
            case FLARE -> MABlocks.FLARE_PRESSURIZED_TUBE;
            case NEURAL -> MABlocks.NEURAL_PRESSURIZED_TUBE;
            case ETERNAL -> MABlocks.ETERNAL_PRESSURIZED_TUBE;
        });
    }
    ...
    private <CHEMICAL extends Chemical<CHEMICAL>, STACK extends ChemicalStack<CHEMICAL>, TANK extends IChemicalTank<CHEMICAL, STACK>> IChemicalTankHolder<CHEMICAL, STACK, TANK> getHolder(BiFunction<MABoxedPressurizedTube, Direction, List<TANK>> tankFunction) {
        return direction -> {
            MABoxedPressurizedTube tube = getTransmitter();
