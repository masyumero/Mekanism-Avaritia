...
public class TileEntityMAThermodynamicConductor extends TileEntityMATransmitter {

    private final HeatHandlerManager heatHandlerManager;

    public TileEntityMAThermodynamicConductor(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
        super(blockProvider, pos, state);
        addCapabilityResolver(heatHandlerManager = new HeatHandlerManager(direction -> {
            MAThermodynamicConductor conductor = getTransmitter();
    ...
    @Override
    protected MAThermodynamicConductor createTransmitter(IBlockProvider blockProvider) {
        return new MAThermodynamicConductor(blockProvider, this);
    }

    @Override
    public MAThermodynamicConductor getTransmitter() {
        return (MAThermodynamicConductor) super.getTransmitter();
    }
    ...
    protected BlockState upgradeResult(@NotNull BlockState current, @NotNull MATier tier) {
        return BlockStateHelper.copyStateData(current, switch (tier) {
            case PRISMATIC -> MABlocks.PRISMATIC_THERMODYNAMIC_CONDUCTOR;
            case FLARE -> MABlocks.FLARE_THERMODYNAMIC_CONDUCTOR;
            case NEURAL -> MABlocks.NEURAL_THERMODYNAMIC_CONDUCTOR;
            case ETERNAL -> MABlocks.ETERNAL_THERMODYNAMIC_CONDUCTOR;
        });
    }
