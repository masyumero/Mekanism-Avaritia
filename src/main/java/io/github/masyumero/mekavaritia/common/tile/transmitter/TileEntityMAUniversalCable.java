...
public class TileEntityMAUniversalCable extends TileEntityMATransmitter implements IComputerTile {

    private final EnergyHandlerManager energyHandlerManager;

    public TileEntityMAUniversalCable(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
        super(blockProvider, pos, state);
        addCapabilityResolver(energyHandlerManager = new EnergyHandlerManager(direction -> {
            MAUniversalCable cable = getTransmitter();
    ...
    @Override
    protected MAUniversalCable createTransmitter(IBlockProvider blockProvider) {
        return new MAUniversalCable(blockProvider, this);
    }

    @Override
    public MAUniversalCable getTransmitter() {
        return (MAUniversalCable) super.getTransmitter();
    }
    ...
    protected BlockState upgradeResult(@NotNull BlockState current, @NotNull MATier tier) {
        return BlockStateHelper.copyStateData(current, switch (tier) {
            case PRISMATIC -> MABlocks.PRISMATIC_UNIVERSAL_CABLE;
            case FLARE -> MABlocks.FLARE_UNIVERSAL_CABLE;
            case NEURAL -> MABlocks.NEURAL_UNIVERSAL_CABLE;
            case ETERNAL -> MABlocks.ETERNAL_UNIVERSAL_CABLE;
        });
    }
